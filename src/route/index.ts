...
export function initApiServer(mercuryClient: MercuryClient) {
  ...
  server.register((instance, _opts, next) => {
    instance.route({
      ...
      handler: async (request: FastifyRequest<{Params: {['pub-key']: string}}>, reply) => {
    ...
    instance.route({
      ...
      handler: async (request: FastifyRequest<{Body: { contract_id: string, pub_key: string }}>, reply) => {
        const { contract_id, pub_key } = request.body
        const { data, error } =  await mercuryClient.addNewTokenSubscription(contract_id, pub_key)
        if (error) {
          reply.code(400).send(error)
        } else {
          reply.code(200).send(data)
    ...
    instance.route({
      ...
      handler: async (request: FastifyRequest<{Body: { pub_key: string }}>, reply) => {
        const { pub_key } = request.body
        const { data, error } =  await mercuryClient.addNewAccountSubscription(pub_key)
        if (error) {
          reply.code(400).send(error)
        } else {
          reply.code(200).send(data)
