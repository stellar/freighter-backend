...
| Price worker     | `GET /api/v1/price-worker-health`           | `200 {"status":"healthy"}`           | Returns `503` when Redis is missing, Horizon is unhealthy, cache bootstrap failed, or price data is stale |
