...
| `FREIGHTER_TRUST_PROXY_RANGE`                                                    | Fastify proxy trust range       | The key must be declared in `.env` or `process.env` (it is in `ENV_KEYS` and absence crashes startup with `INVALID_ENV`). Leave the value empty to use the built-in default of `loopback,linklocal,uniquelocal` (covers typical k8s / load-balancer topologies). Set to a CIDR (e.g. `172.16.0.0/12`) to restrict trust; any non-empty invalid value is passed to `proxy-addr` and will crash the process at startup. Misconfiguration causes `/onramp/token` to fail closed with a 400. |
