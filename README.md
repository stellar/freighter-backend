...
| Kubernetes with a known VPC CIDR you want to lock down           | explicit CIDR, e.g. `172.16.0.0/12`                                                                                                              | Stricter than the default — only the specific proxy CIDR is trusted. SDF prod uses this shape.                                                                                                                                                                                                                                                                                                                 |
...
**Required observability for any deployment:**

- Alert on the warning log line `"Refusing to issue an unbound Coinbase session"` (any non-zero rate is a misconfig signal — real users should never produce this).
